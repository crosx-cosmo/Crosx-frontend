...
import { Copy, HelpCircle, Radio, Save } from "lucide-react";
...
import { POSTBACK_EVENTS, POSTBACK_TOTALS, POSTBACK_URL } from "@/lib/publisher-data";
            ...
            <tbody>
              {CAMPAIGN_MACROS.map((m) => (
                <Tr key={m.macro}>
                  ...
                  <Td>
                    <code className="block max-w-[420px] truncate font-mono text-[11.5px] text-muted-foreground">
                      {m.example}
                    </code>
                  </Td>
                  <Td className="text-right">
                    <span className="inline-flex justify-end gap-2">
                      ...
                      <ActionButton
                        ...
                        icon={Copy}
                        onClick={async () => {
                          const ok = await copyText(m.example);
                          if (ok) toast.success(`${m.macro} example copied`);
                          else toast.error("Unable to complete action");
                        }}
                      >
                        Example

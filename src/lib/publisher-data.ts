...
export type SubIds = Partial<
  Record<"x1" | "x2" | "x3" | "x4" | "x5" | "x6" | "x7" | "x8" | "x9" | "x10", string>
>;
...
export type LeadRecord = {
  ...
  date: string;
  /** Conversion event that produced this lead. */
  event: string;
  clickId: string;
...
const LEAD_EVENTS = ["Registration", "KYC Completed", "First Trade", "App Install"];
const LEAD_DEVICES = ["Android", "iOS", "Desktop", "Android"];
const LEAD_OS = ["Android 14", "iOS 17.4", "Windows 11", "Android 13"];
const LEAD_CITIES = ["Mumbai", "Bengaluru", "Delhi", "Pune", "Hyderabad", "Kolkata"];
const LEAD_SOURCES = ["Facebook Ads", "Google UAC", "Native", "Email", "SEO"];
const LEAD_QUALIFICATION: Record<LeadRecord["status"], string> = {
  Qualified: "Advertiser validated the lead — payout approved and queued for the next cycle.",
  Pending: "Awaiting advertiser verification. Validation usually completes within 3-5 days.",
  Rejected: "Rejected by advertiser — duplicate or invalid contact details.",
};

export const LEAD_RECORDS: LeadRecord[] = Array.from({ length: 36 }, (_, i) => {
  const campaign = CAMPAIGNS[i % 4]!;
  const status = LEAD_STATUS[i % LEAD_STATUS.length]!;
  const date = i < 10 ? "Aug 08" : i < 24 ? "Aug 07" : "Aug 06";
  const time = `${String(9 + (i % 12)).padStart(2, "0")}:${String((i * 7) % 60).padStart(2, "0")}`;
  return {
    id: `LD-${82921 - i}`,
    campaign: campaign.name,
    ...
    status,
    date,
    event: LEAD_EVENTS[i % LEAD_EVENTS.length]!,
    clickId: `clk_${(918273645 - i * 137).toString(36)}`,
    transactionId: `txn_${((std_hash(i) % 9_000_000) + 1_000_000).toString()}`,
    device: LEAD_DEVICES[i % LEAD_DEVICES.length]!,
    os: LEAD_OS[i % LEAD_OS.length]!,
    geo: campaign.geo,
    city: LEAD_CITIES[i % LEAD_CITIES.length]!,
    trafficSource: LEAD_SOURCES[i % LEAD_SOURCES.length]!,
    subIds: {
      x1: LEAD_SOURCES[i % LEAD_SOURCES.length]!.toLowerCase().replace(/\s+/g, "_"),
      x2: `camp_${1000 + (i % 12)}`,
      x3: `creative_${(i % 6) + 1}`,
      x4: `placement_${(i % 4) + 1}`,
      x5: `${LEAD_CITIES[i % LEAD_CITIES.length]!.toLowerCase()}`,
    },
    clickedAt: `2026-${date === "Aug 08" ? "08-08" : date === "Aug 07" ? "08-07" : "08-06"} ${time}`,
    leadAt: `2026-${date === "Aug 08" ? "08-08" : date === "Aug 07" ? "08-07" : "08-06"} ${time.replace(/^(\d\d)/, (m) => String(Number(m) + 1).padStart(2, "0"))}`,
    reviewedAt:
      status === "Pending"
        ? null
        : `2026-${date === "Aug 08" ? "08-09" : date === "Aug 07" ? "08-08" : "08-07"} 11:${String((i * 3) % 60).padStart(2, "0")}`,
    qualification: LEAD_QUALIFICATION[status],
  };
});

function std_hash(n: number) {
  return (n + 1) * 7919;
}

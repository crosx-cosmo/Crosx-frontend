...
const santanuPhotoUrl =
  "https://xtprixndtsnctglngxwl.supabase.co/storage/v1/object/public/email-assets/IMG-20260829-WA0001.jpg";
  ...
  return (
    <Section ariaLabel="Leadership team">
      ...
      <ul className="mx-auto mt-14 grid max-w-4xl gap-5 sm:grid-cols-2">
        {TEAM.map((member, i) => {
          const isSantanu = i === 0;
          const CardWrapper = isSantanu ? motion.article : "article";
          const cardProps = isSantanu
            ? {
                tabIndex: 0,
                whileHover: { y: -6, scale: 1.01 },
                whileTap: { scale: 0.995 },
                whileFocus: { y: -4, scale: 1.005 },
          ...
          return (
            <Reveal as="li" key={member.name} delay={i * 0.06}>
              <CardWrapper
                className="glass group relative h-full overflow-hidden rounded-3xl p-7 will-change-transform"
                {...cardProps}
              >
                <span
                  aria-hidden="true"
                  className="pointer-events-none absolute -right-16 -top-16 size-48 rounded-full bg-brand/10 blur-[80px] opacity-0 transition-opacity duration-500 group-hover:opacity-100 group-focus-visible:opacity-100"
                />
                {isSantanu && (
                  <span
                    aria-hidden="true"
                    className="pointer-events-none absolute inset-0 rounded-3xl opacity-0 shadow-[0_0_0_1px_hsl(var(--brand)/0.0),0_20px_60px_-20px_hsl(var(--brand)/0.35)] transition-opacity duration-500 group-hover:opacity-100 group-focus-visible:opacity-100"
                  />
                )}
                <div className="relative flex items-center gap-4">
                  {member.photo ? (
                    <span className="relative grid size-14 shrink-0 place-items-center rounded-full border border-brand/40 bg-surface-2 p-[2px] shadow-[0_0_24px_-6px_hsl(var(--brand)/0.45)] transition-shadow duration-500 group-hover:shadow-[0_0_32px_-4px_hsl(var(--brand)/0.6)] group-focus-visible:shadow-[0_0_32px_-4px_hsl(var(--brand)/0.6)]">
                      <img
                        src={member.photo}
                        alt={`${member.name}, ${member.role} of CrosX`}

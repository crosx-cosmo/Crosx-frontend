...
export function Select({
  ...
  const selected =
    options.find((o: { value: string; label: string }) => o.value === String(value ?? "")) ??
    options[0];
  ...
  return (
    <div ref={ref} className={cn("relative min-w-0", className)}>
      ...
      {mounted ? createPortal(<AnimatePresence>{menu}</AnimatePresence>, document.body) : null}
    </div>
  );
}
...
export const stickyHeadCell =
  "sticky left-0 z-30 bg-surface shadow-[6px_0_10px_-8px_rgba(0,0,0,0.35)]";
export const stickyCell = "sticky left-0 z-20 bg-surface shadow-[6px_0_10px_-8px_rgba(0,0,0,0.35)]";
...
export function TableWrap({
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const measure = () => {
      const cell = el.querySelector<HTMLElement>("thead th.sticky, thead td.sticky");
      setStickyWidth(cell ? cell.getBoundingClientRect().width : 0);
  ...
  const veilOpacity = Math.min(1, scrollLeft / 36);

  return (
    <div className="relative max-w-full min-w-0">
      <div ref={scrollRef} className="max-w-full min-w-0 overflow-x-auto overscroll-x-contain">
      ...
      {stickyWidth > 0 && (
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-y-0 z-[15] backdrop-blur-[3px]"
          style={{
            left: stickyWidth,
            width: 56,
            opacity: veilOpacity,
            ...
            background:
              "linear-gradient(to right, color-mix(in oklab, var(--surface) 88%, transparent), color-mix(in oklab, var(--surface) 0%, transparent))",
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3" aria-hidden="true">
      {Array.from({ length: count }).map((_, i) => (
        <div
          key={i}
          className="glass h-32 animate-pulse rounded-2xl bg-surface-2/40 [animation-duration:0.7s]"
        />
      ))}
    </div>
  );
...
export function Val({ value, width = "w-28" }: { value?: string | null; width?: string }) {

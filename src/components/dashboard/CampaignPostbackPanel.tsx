...
import { Check, Copy, ExternalLink, Globe2, HelpCircle, Info, Link2, Zap } from "lucide-react";
...
import { PostbackSpeedAnalyzer } from "@/components/dashboard/PostbackSpeedAnalyzer";
import { usePublisherMock } from "@/components/dashboard/mock-store";
import { CAMPAIGN_MACROS, conversionEventsFor } from "@/lib/campaign-postback-data";
  ...
  return (
    ...
    >
      <div className="grid gap-5">
        <div className="grid gap-2.5 rounded-2xl border border-brand/35 bg-brand/8 p-4 shadow-[0_0_28px_-16px_var(--color-brand)]">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <span className="inline-flex items-center gap-2 font-display text-sm font-black tracking-tight">
              <Link2 className="size-4 text-brand" aria-hidden="true" />
              Primary Tracking URL
              <StatusBadge tone={campaign.status === "Paused" ? "danger" : "success"} dot>
          ...
          <p className="text-[12.5px] text-muted-foreground">
            Send all traffic and leads for{" "}
            <span className="font-bold text-foreground">{campaign.name}</span> through this link —
            clicks are only tracked on this URL.
          </p>
        </div>
...
/** Delivery diagnostics for this campaign's active postback endpoint. */
export function CampaignSpeedAnalyzer() {
  const { global } = useCampaignPostbacks();
  return <PostbackSpeedAnalyzer endpoint={global.url} method="S2S" />;
}

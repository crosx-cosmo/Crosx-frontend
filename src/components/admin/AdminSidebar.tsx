...
import { ADMIN_NAV, isGroup, type NavItem } from "./nav-config";
        ...
        <span
          className={cn(
            ...
            active
              ...
              : "border-hairline bg-surface-2/70 text-muted-foreground",
          )}
        >
          {badge}
        </span>
...
function NavTree({ collapsed, onNavigate }: { collapsed: boolean; onNavigate?: () => void }) {
  ...
  const [open, setOpen] = useState<Record<string, boolean>>(() =>
    Object.fromEntries(ADMIN_NAV.filter(isGroup).map((g) => [g.label, groupContains(g, path)])),
  );
  ...
  return (
    <nav aria-label="Admin dashboard" className="flex flex-col gap-1 px-2">
      {ADMIN_NAV.map((item) => {
        if (!isGroup(item)) {
          return (
            <Link
    ...
    >
      ...
      {!collapsed && (
        <div className="glass m-3 rounded-2xl p-4">
          ...
          <p className="mt-1 font-display text-sm font-bold">Super Admin</p>
          <p className="mt-1 text-xs text-muted-foreground">Full platform control</p>
        </div>
      )}
    </aside>
  );
